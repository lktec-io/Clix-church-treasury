...
// signed-out visitor. Currently just /login: there is no public /register
// (every tenant is created by a Platform Administrator via
// /platform/tenants — see auth.routes.js). An already-authenticated user
// hitting /login is sent to their landing page instead of being shown the
// form again.
//
// Deliberately does NOT wait on status === 'checking' the way
// ProtectedRoute does: a protected page waiting briefly to avoid a
// login-flash for an already-authenticated user is worth it, but the one
// public page must never be held up by the silent refresh-token check —

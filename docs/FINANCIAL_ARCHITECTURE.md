...
- **Phase 11 finding, fixed:** the "never a floating-point money value" rule held for every SQL-level SUM, but was violated in three places that summed an *already-fetched row list* in JS (`Number(a) + Number(b)`) rather than via SQL — the pledge-overpayment guard in `contributions.service.js`, and two report totals in `reports.service.js`. `money.js` gained `addMoney`/`compareMoney`/`sumMoney`, all integer-cents arithmetic, and all three call sites were switched over; a `+ 0.001` epsilon that had been added specifically to paper over float rounding in the overpayment check was removed as no longer needed. See [SECURITY_ARCHITECTURE.md](SECURITY_ARCHITECTURE.md) §9 for the full writeup and regression test.

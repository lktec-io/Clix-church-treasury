...
## Post-Launch Incident — Audit Log JSON Serialization Failure

Reported against the live production database (`treasurer`, 25 migrations applied, 36 permissions seeded): PM2 logs showed `ER_INVALID_JSON_TEXT` on `audit_logs.after_state`/`before_state` for `financial_period.reopened`, `user.role_assigned`, and `user.role_removed`.

**Root cause:** `auditLogRepository.insert()` bound a caller's plain JS object directly as a `?` query parameter for a `JSON` column. `mysql2` does not auto-encode a plain object bound this way — it falls back to `String(value)`, producing the literal text `"[object Object]"`, which MySQL rejects. Every one of the ~30 `recordAuditLog()` call sites across the codebase passes `before`/`after` as a plain object (confirmed by reading all of them, not assumed) — the three actions in the error log were simply the first ones exercised against production traffic; the same crash was latent in every other object-valued audit call too (`account.*`, `fund.*`, `budget.*`, `pledge.*`, `contribution.*`, `expense.*`, `transaction.*`).

**Fix — centralized, not patched per-caller:** `auditLogRepository.js` now exports `serializeAuditState(value)`, the single point a JS value becomes JSON-column text: objects/arrays → `JSON.stringify` (with recursive redaction of any key matching `/password|token|secret|jwt|hash|apikey|api_key|credential/i` → `[REDACTED]`, defense-in-depth — no current caller was found to pass a sensitive field, confirmed by reading every call site); `null`/`undefined` → SQL `NULL`; an already-valid JSON string passes through unchanged; a plain non-JSON string is wrapped as a JSON string value. Verified by direct execution (not just review): every case (object, null, undefined, valid-JSON string, plain string, nested object, array, sensitive-key redaction) produces valid, parseable JSON — see `tests/phase11/auditSerialization.test.js`, and a standalone `node -e` execution during this fix that confirmed the same, independent of the (still-unreachable) test database.

**Transaction/atomicity:** reviewed per the incident brief. Financially-critical postings already run their audit write inside the same DB transaction as the state change (unchanged, correct, by design). Simpler state changes (period close/reopen, account/fund mutations, role assign/remove) do not — a pre-existing, consistent pattern across every phase, not introduced by this fix, and assessed as out of scope to redesign here (see [SECURITY_ARCHITECTURE.md](SECURITY_ARCHITECTURE.md) §7 for the full reasoning). Once serialization is fixed, this stops mattering in practice for the reported incident, since the audit write now succeeds.

**RBAC / 403 investigation:** reviewed the entire chain — `seedRbacCatalog()` (idempotent, expands `'ALL'` against the full current permission catalog, uses `INSERT IGNORE` so re-seeding after new permissions were added in later phases correctly back-fills them), `auth.service.js#registerTenant` (assigns Super Administrator atomically inside the same transaction as tenant/user creation), `permissionsRepository.listForUser` (a straightforward three-table JOIN, re-derived from the DB on every request, never trusted from the JWT). **Found no code defect in this chain.** Added a new regression test (`tests/phase11/auditSerialization.test.js`) that exercises the *exact* production path — `POST /auth/register-tenant` → `POST /auth/login` → `GET /auth/me` — and asserts the returned permission list has all 36 entries, including specific permissions from every phase (`financial_period.reopen`, `users.manage`, `reports.export`, `audit.view`), not just a non-empty check. The most likely explanation for the reported 403s is the same incident: `user.role_assigned`/`.removed` were crashing with a 500 *after* the underlying `user_roles` row had already been written (the state change and the audit write are two separate, sequential statements — see above), which could easily read as "role assignment failed" when it may have actually succeeded, or vice versa produce confusion about which role a test account actually holds. This is a hypothesis consistent with the evidence, not a confirmed cause — live production `user_roles`/`role_permissions` data was not inspected (no database credentials were available or requested). **Recommended concrete next step:** after redeploying this fix, call `GET /api/v1/auth/me` as the actual production Super Administrator account and directly read its `permissions` array — this is the one live check that resolves the question definitively, and it's safe, read-only, and requires no code change to perform.

**Status: IMPLEMENTED (serialization fix). RBAC code re-verified with no defect found and new regression coverage added. LIVE VERIFICATION: PENDING — the specific A–E checks (reopen period / assign role / remove role / confirm valid JSON / confirm no 500) require redeploying this fix to the production host, which was not done as part of this change (no server access, none requested).**

---

## Post-Launch Incident — Browser Calling http://localhost:4005 in Production

Reported against the live site: the browser called `http://localhost:4005/api/v1/auth/login`/`/auth/refresh` instead of `https://treasurer.clixworks.co.tz/api/v1`, causing `ERR_CONNECTION_REFUSED`. **No SSH/server access was available to diagnose or fix this live** — this entry documents what was found and fixed in the repository, and what still requires the operator to run the redeploy commands themselves.

**Root cause:** Vite bakes `import.meta.env.VITE_API_BASE_URL` into the JS bundle at *build* time. No `.env.production` file existed anywhere in the repository — the only place the correct production URL lived was as an inline prefix documented in `docs/DEPLOYMENT.md` (`VITE_API_BASE_URL=... npm run build`), and as the *active default value* inside the committed `.env.example` (`VITE_API_BASE_URL=http://localhost:4005/api/v1`, since `.env.example` doubles as the local-dev template). `src/api/client.js` additionally had a hardcoded `?? 'http://localhost:4005/api/v1'` fallback for whenever the env var was absent. Put together: any build invoked as plain `npm run build` — without the inline prefix, e.g. a re-run after `git pull`, a differently-written deploy script, or simply forgetting it — silently baked the hardcoded localhost fallback into the bundle instead of failing loudly. The evidence matched exactly: one earlier build (`index-CzLXLEnX.js`) had the correct URL (built with the inline prefix), a later one (`index-D0ymvqsy.js`) did not.

**Fix — two parts, both permanent, neither dependent on remembering a step at build time:**
1. **`.env.production`** (new, repo root, *deliberately committed* — not a secret, every Vite `VITE_`-prefixed var is bundled into public JS regardless) sets `VITE_API_BASE_URL=https://treasurer.clixworks.co.tz/api/v1`. Vite loads this automatically for every `vite build` (production mode is the default), so `npm run build` alone — no inline prefix — is now always correct. `.gitignore`'s blanket `.env.*` exclusion was given an explicit `!.env.production` exception, mirroring the existing `!.env.example` one.
2. **`src/api/client.js`'s fallback made mode-aware** rather than a single hardcoded value: `import.meta.env.DEV` (Vite's own build-time flag, true only for `vite dev`) selects between the local dev port fallback and, for any built bundle, `${window.location.origin}/api/v1` — same-origin, which is *actually correct* for this architecture's production topology (frontend and API always share an origin behind Nginx) rather than a guessed port. This is defense-in-depth: even if `.env.production` were somehow missing from a future checkout, a production build would self-heal to the right URL instead of repeating this exact incident.

**Verified by actually re-running the previously-broken command:** `rm -rf dist && npm run build` (plain, no inline var — the exact command that produced the broken bundle) was re-run in the development environment; the output was grepped and confirmed to contain `treasurer.clixworks.co.tz/api/v1` with zero `localhost:4005`/`localhost:4000` matches. This is real, executed verification of the fix's mechanism — not a claim that the *live* production site was checked (it wasn't; see below).

**Also found, corrected, and flagged as a possible independent contributor:** `docs/DEPLOYMENT.md` and `deploy/nginx.conf.example` both suggested a production directory of `/var/www/treasurer.clixworks.co.tz` — this deployment's actual, already-established production directory is `/var/www/treasurer` (confirmed by the operator; intentionally not domain-named, and not the git repo's name either). Both files corrected to `/var/www/treasurer` throughout, with the git clone step updated to the real repository URL (`https://github.com/lktec-io/Clix-church-treasury.git`). **If the live Nginx `root` directive was ever set up following the old suggested path while the actual deploy script copies fresh builds to a different path, Nginx would keep serving a stale bundle indefinitely regardless of how correct any new build is** — this cannot be confirmed or ruled out without inspecting the live Nginx config, which requires server access this environment does not have. The corrected `deploy/nginx.conf.example` now includes an explicit instruction to verify `root` against the actual deploy path via `sudo nginx -T`.

**What was NOT done, and could not be done, from this environment:** no SSH/server access exists. Nothing was executed on the VPS — no `git pull`, no rebuild, no `pm2 restart`, no `nginx -t`/`reload`, no live `curl` (a `WebFetch` attempt against the live domain returned `403 Forbidden`, almost certainly Cloudflare bot protection, confirming no working live-verification channel exists from this environment either). The fix exists in this repository and is committed; **it is not live until the operator pulls this commit on the VPS and runs the redeploy steps in `docs/DEPLOYMENT.md` §L ("Redeploying later").**

**Status: IMPLEMENTED (repository-side fix, verified by direct re-execution of the build). LIVE VERIFICATION: PENDING — requires the operator to redeploy on the actual VPS; not fabricated, not assumed.**
